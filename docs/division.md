...
| 1.2 配置路由与基础布局                          | **A**    | 在 `App.tsx`、`src/router` 中配置 Login / Register / HotelEdit / HotelReviewList 占位 |
| 1.3 封装 `request.ts`、`auth.ts`、`hotel.ts`   | **A**    | 包括基础 Axios 实例、请求拦截/响应拦截、错误处理                                      |
| 1.4 定义基础类型与常量                          | **A**    | 在 `src/types`、`src/constants` 中定义 User、Hotel、ReviewStatus 等类型与枚举         |
| 1.5 登录/注册页面骨架                           | **A**    | 基本表单结构与按钮，暂不接真实接口                                                   |
| 1.6 酒店信息编辑页面骨架                        | **A**    | 表单大致分区（基础信息、价格、房型等）占位                                            |
